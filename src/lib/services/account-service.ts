...
import { defaultChartOfAccounts, type ChartOfAccountsItemDefinition } from '../accounting/chartOfAccounts';
...
export class AccountService {
   ...
   */
  private boolToDb(value: boolean | undefined, defaultValue: boolean = false): number {
   ...
   */
  private percentageToBasisPoints(value: number | null | undefined): number | null {
  ...
  async getAccountsByType(userId: string, type: DbAccountSystemType): Promise<DbChartOfAccount[]> {
    ...
    try {
      const results = await this.db.query<DbChartOfAccount>(sql, [userId, type]);
      return results.map(this.mapDbToApp);
    } catch (error: unknown) {
  ...
  async getAccountById(id: string, userId: string): Promise<DbChartOfAccount | null> {
    ...
    try {
      const account = await this.db.queryOne<DbChartOfAccount>(sql, [id, userId]);
  ...
  async getAccountByCode(code: string, userId: string): Promise<DbChartOfAccount | null> {
    ...
    try {
      const account = await this.db.queryOne<DbChartOfAccount>(sql, [code, userId]);
      return account ? this.mapDbToApp(account) : null;
    } catch (error: unknown) {
   ...
   */
  async isAccountUsedInTransactions(accountId: string, userId: string): Promise<boolean> {
    const sql = `
    ...
    try {
      const result = await this.db.queryOne<{1: number}>(sql, [accountId, userId]);
      return !!result;
    } catch (error: unknown) {
   ...
   */
  async checkCircularDependency(accountId: string, parentId: string, userId: string): Promise<boolean> {
  ...
  async createAccount(accountData: AccountServiceInput, userId: string): Promise<DbChartOfAccount> {
    ...
    const dbRecoveryPercentage = this.percentageToBasisPoints(accountData.recovery_percentage);
    ...
    const params = [
      ...
      dbIsRecoverable, dbRecoveryPercentage, accountData.tax_category || null,
      dbIsActive, accountData.parent_id || null,
      now, now
    ];
  ...
  async updateAccount(accountId: string, accountData: Partial<AccountServiceInput>, userId: string): Promise<DbChartOfAccount> {
    const existingAccount = await this.getAccountById(accountId, userId);
    if (!existingAccount) throw new NotFoundError('Account not found or access denied.');
    ...
    if (accountData.code && accountData.code !== existingAccount.code) {
      ...
      if (conflictingAccount && conflictingAccount.id !== accountId) {
        throw new ConflictError(`Account code "${accountData.code}" is already in use.`);
      }
    }
    
    if (accountData.parent_id !== undefined) { // Check if parent_id is part of the update
      if (accountData.parent_id === accountId) {
        throw new ValidationError('Account cannot be its own parent.');
      }
      
      if (accountData.parent_id !== null) { // Only validate if it's not being set to null
        const parentAccount = await this.getAccountById(accountData.parent_id, userId);
        if (!parentAccount) {
          throw new ValidationError('Parent account not found or access denied.');
        }
        ...
        const hasCircular = await this.checkCircularDependency(accountId, accountData.parent_id, userId);
        if (hasCircular) {
          throw new ValidationError('Cannot set parent account: would create a circular dependency');
        }
      }
    }
    ...
    const addUpdateField = (key: keyof AccountServiceInput, dbKey: keyof DbChartOfAccount, transformToDbFormat?: (val: any) => any) => {
      if (key in accountData && accountData[key] !== undefined) {
        const appValue = accountData[key];
        const dbValue = transformToDbFormat ? transformToDbFormat(appValue) : appValue;
        // Check if new value is different from existing DB value
        if (dbValue !== existingAccount[dbKey] || (dbValue === null && existingAccount[dbKey] !== null)) {
          updateFields.push(`${dbKey.toString()} = ?${updateValues.length + 1}`);
    ...
    addUpdateField('recovery_percentage', 'recovery_percentage', (v) => this.percentageToBasisPoints(v));
    ...
    try {
      ...
      if (!result.success) { /* Optional: Check result.meta.changes, could be 0 if data is same */ }
  ...
  async deleteAccount(accountId: string, userId: string): Promise<boolean> {
    ...
    if (!account) throw new NotFoundError('Account not found or access denied.');

    // Check for child accounts
    const childAccountsSql = `SELECT id FROM ${this.TABLE_NAME} WHERE parent_id = ?1 AND user_id = ?2 LIMIT 1`;
    const childAccount = await this.db.queryOne<{id: string}>(childAccountsSql, [accountId, userId]);
    if (childAccount) {
      throw new ValidationError('Cannot delete account that is a parent. Reassign children first.');
    }
    
    // Check if account is used in transactions
    const isUsedInTransactions = await this.isAccountUsedInTransactions(accountId, userId);
    if (isUsedInTransactions) {
      throw new ValidationError('Cannot delete account that is used in transactions.');
    }
    ...
    const entityAccountsSql = `
      ...
      LIMIT 1
    `;
    const entityAccount = await this.db.queryOne<{id: string}>(entityAccountsSql, [accountId, userId]);
    if (entityAccount) {
      throw new ValidationError('Cannot delete account that is linked to entities. Remove entity associations first.');
    }
  ...
  async batchUpdateAccounts(
    ...
    for (const update of accountUpdates) {
      ...
      if (update.data.parent_id !== undefined && update.data.parent_id !== null) {
        // Skip self-references
        if (update.data.parent_id === update.id) continue;
        
        // Validate parent exists
        const parentAccount = await this.getAccountById(update.data.parent_id, userId);
        if (!parentAccount) continue;
        
        // Check circular dependency
        const hasCircular = await this.checkCircularDependency(update.id, update.data.parent_id, userId);
      ...
      const addField = (key: keyof AccountServiceInput, dbKey: string, transform?: (v: any) => any) => {
        if (key in update.data && update.data[key] !== undefined) {
          const value = update.data[key];
          updateFields.push(`${dbKey} = ?${paramIndex}`);
          paramIndex++;
          updateValues.push(transform ? transform(value) : (value === undefined ? null : value));
        }
      };
      ...
      addField('is_recoverable', 'is_recoverable', v => this.boolToDb(v, false));
      addField('recovery_percentage', 'recovery_percentage', v => this.percentageToBasisPoints(v));
  ...
  async initializeDefaultAccounts(userId: string): Promise<number> {
    ...
    if (defaultChartOfAccounts.some(defAcc => existingUserCoa.find(exAcc => exAcc.code === defAcc.code && exAcc.user_id === userId))) {
      console.log(`Default accounts likely exist for user ${userId}. Skipping full initialization.`);
      return 0;
    }
    ...
    const preparedDefaults: Array<ChartOfAccountsItemDefinition & { generatedId: string; dbParentId: string | null }> = [];
    ...
    for (const defAcct of defaultChartOfAccounts) {
        ...
        preparedDefaults.push({ ...defAcct, generatedId: accountId, dbParentId: null });
    }
    ...
    for (const accToInsert of preparedDefaults) {
        if (accToInsert.parentCode) {
            accToInsert.dbParentId = codeToIdMap.get(accToInsert.parentCode) || null;
        }
        ...
        accountsToBatch.push(
          this.db.d1Instance.prepare(sql).bind(
            ...
            dbIsRecoverable, dbRecoveryPercentage,
    ...
    try {
      ...
      results.forEach(result => {
        if (result.success && result.meta?.changes) insertedCount += result.meta.changes;
        else if (!result.success) console.warn('Failed to insert a default account:', result.error);
      });
  ...
  async getAccountHierarchy(userId: string): Promise<Array<DbChartOfAccount & { children: Array<DbChartOfAccount & { children: any[] }> }>> {
    const allAccountsFlat = await this.getAllAccounts(userId);
    if (!allAccountsFlat || allAccountsFlat.length === 0) return [];

    type HierarchicalAccount = DbChartOfAccount & { children: HierarchicalAccount[] };
    const accountsById = new Map<string, HierarchicalAccount>();
    allAccountsFlat.forEach(acc => accountsById.set(acc.id, { ...this.mapDbToApp(acc), children: [] }));
