...
type ReportParams = BalanceSheetParams | IncomeStatementParams | CashFlowParams | TaxSummaryParams;
...
export async function generateReport(
  ...
  switch (reportType) {
    case ReportType.BALANCE_SHEET:
      return generateBalanceSheetJS(accounts, (params as BalanceSheetParams).date);
...
function generateCashFlowReportJS(
  ...
  Object.entries(accounts).forEach(([accountId, account]) => {
    if (account.type === 'asset' && account.name.toLowerCase().includes('cash')) {
  ...
  transactions.forEach(transaction => {
    ...
    for (const line of lines) {
      ...
      if (accountType === 'asset' && (accountName.includes('equipment') || accountName.includes('property'))) {
        category = 'investing';
        break;
      } else if (accountType === 'liability' && accountName.includes('loan')) {
        category = 'financing';
        break;
      } else if (accountType === 'equity' && (accountName.includes('capital') || accountName.includes('draw'))) {
        category = 'financing';
        break;
      }
    }
  ...
  const totalCashFlow = totalOperating.plus(totalInvesting).plus(totalFinancing);
...
function generateTaxSummaryJS(
  ...
  const incomeStatement = generateIncomeStatementJS(accounts, transactions, startDate, endDate);
  ...
  expenseItems.forEach(expense => {
    ...
    if (name.includes('depreciation')) {
      depreciationExpense = depreciationExpense.plus(new Decimal(expense.balance));
    }
  });
