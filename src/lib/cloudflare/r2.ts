...
export class R2Client {
  ...
  async upload(
    ...
    try {
      ...
      if (options.contentType) uploadOptions.httpMetadata = { 
        ...uploadOptions.httpMetadata,
        contentType: options.contentType 
      };
      if (options.cacheControl) uploadOptions.httpMetadata = { 
        ...uploadOptions.httpMetadata,
        cacheControl: options.cacheControl 
      };
      if (options.contentDisposition) uploadOptions.httpMetadata = { 
        ...uploadOptions.httpMetadata,
        contentDisposition: options.contentDisposition 
      };
      if (options.contentEncoding) uploadOptions.httpMetadata = { 
        ...uploadOptions.httpMetadata,
        contentEncoding: options.contentEncoding 
      };
      if (options.contentLanguage) uploadOptions.httpMetadata = { 
        ...uploadOptions.httpMetadata,
   ...
   */
  async download(key: string, options: { asStream?: boolean } = {}): Promise<R2ObjectBody | null> {
    try {
      // Get the object
  ...
  async list(options: R2ListOptions = {}): Promise<R2ListResult> {
    try {
      ...
      const objects = result.objects.map(obj => this.mapObjectHeadToInfo(obj.key, obj));
  ...
  async copy(
    ...
    try {
      ...
      for (const key of ['cacheControl', 'contentDisposition', 'contentEncoding', 'contentLanguage']) {
      ...
      const result = await this.bucket.put(destinationKey, source.body, uploadOptions);
  ...
  async getPresignedUrl(
    ...
    try {
      ...
      // Generate the URL
      const url = await this.bucket.createPresignedUrl(
        key,
        presignOptions
      );
   ...
   */
  private mapObjectHeadToInfo(key: string, obj: R2Object | R2ObjectBody): R2ObjectInfo {
    return {
      key,
      size: obj.size,
      etag: obj.etag,
      ...
      checksums: {
        md5: obj.checksums?.md5 ? this.bytesToHex(obj.checksums.md5) : undefined,
        sha1: obj.checksums?.sha1 ? this.bytesToHex(obj.checksums.sha1) : undefined,
        sha256: obj.checksums?.sha256 ? this.bytesToHex(obj.checksums.sha256) : undefined,
        sha512: obj.checksums?.sha512 ? this.bytesToHex(obj.checksums.sha512) : undefined,
      }
    };
  }
  ...
  private wrapError(operation: string, key: string, error: any): Error {
    ...
    const wrappedError = new Error(`R2 ${operation} operation failed for '${key}': ${message}`);
