...
const OPERATING_LIABILITY_TYPES = [
  "ACCOUNTS_PAYABLE",
  "UNEARNED_REVENUE",
  "ACCRUED_LIABILITY",
  "OTHER_CURRENT_LIABILITY",
];
const INVESTING_ASSET_TYPES = [
  "FIXED_ASSET",
  "INVESTMENT",
  "INTANGIBLE_ASSET",
  "OTHER_ASSET",
];
const FINANCING_LIABILITY_TYPES = [
  "LONG_TERM_LIABILITY",
  "LOAN",
  "NOTE_PAYABLE",
];
const FINANCING_EQUITY_TYPES = [
  "EQUITY",
  "COMMON_STOCK",
  "PREFERRED_STOCK",
  "RETAINED_EARNINGS",
...
export async function generateCashFlow(
  ...
  const beginningCash = startingBalances.reduce(
    (sum: number, balance: AccountBalance) => sum + balance.balance,
    0
  );
  ...
  if (calculatedNetIncome === undefined) {
    ...
    const revenues = incomeTotals
      .filter((t: any) =>
        ["REVENUE", "INCOME"].includes(accountMap.get(t.accountId)?.type)
      )
      .reduce((sum: number, t: any) => sum + t.total, 0);

    const expenses = incomeTotals
      .filter((t: any) =>
        ["EXPENSE", "COST_OF_GOODS_SOLD"].includes(
          accountMap.get(t.accountId)?.type
        )
      )
      .reduce((sum: number, t: any) => sum + t.total, 0);
  ...
  if (!isReconciled) {
    console.warn(
      `Cash flow statement doesn't reconcile. Beginning Cash: ${beginningCash}, Net Increase: ${netIncrease}, Calculated Ending: ${calculatedEndingCash}, Actual Ending: ${endingCash}, Difference: ${calculatedEndingCash - endingCash}`
    );
...
export function formatCashFlow(
  ...
  (
    formattedReport.operatingActivities as OperatingActivitiesSection
  ).formattedNetIncome = formatCurrency(
  ...
  formattedReport.operatingActivities.changesInWorkingCapital.forEach(
    (item) => {
      item.formattedAmount = formatCurrency(item.amount, currencyCode);
    }
  );
  ...
  (
    formattedReport.operatingActivities as OperatingActivitiesSection
  ).formattedNetCash = formatCurrency(
  ...
  (
    formattedReport.investingActivities as InvestingActivitiesSection
  ).formattedNetCash = formatCurrency(
  ...
  (
    formattedReport.financingActivities as FinancingActivitiesSection
  ).formattedNetCash = formatCurrency(
...
export function calculateCashFlowRatios(
  ...
  if (
    ...
    if (incomeStatement.netIncome) {
      ratios.operatingCashFlowToNetIncome =
        operatingCashFlow / incomeStatement.netIncome;
    }
  }
  ...
  if (balanceSheet) {
    ...
    if (
      ...
      const totalCurrentLiabilities =
        currentLiabilities[currentLiabilities.length - 1].amount;
