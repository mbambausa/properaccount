...
export async function generateBalanceSheetReport(
  ...
  try {
    ...
    throw new Error(
      `Failed to generate balance sheet: ${(error as Error).message}`
    );
  }
}
...
export async function generateIncomeStatementReport(
  ...
  try {
    ...
    throw new Error(
      `Failed to generate income statement: ${(error as Error).message}`
    );
  }
}
...
export async function generateCashFlowReport(
  ...
  try {
    ...
    throw new Error(
      `Failed to generate cash flow statement: ${(error as Error).message}`
    );
  }
}
  ...
  try {
    ...
    if (options.includeCashPosition) {
      dashboard.cashPosition = {
        ...
        projected: calculateProjectedCashBalance(
          cashFlow.endingCash,
          cashFlow.netIncrease
        ),
        trend: generateTrendData(
          cashFlow.beginningCash,
          cashFlow.endingCash,
          6
        ),
    ...
    throw new Error(
      `Failed to generate dashboard report: ${(error as Error).message}`
    );
  }
}
