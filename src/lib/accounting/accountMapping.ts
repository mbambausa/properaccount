...
export class AccountMappingService {
  ...
  private validateMappingConfig(): void {
    ...
    if (missingAccounts.length > 0) {
      console.warn(`AccountMappingService: The following mapped accounts do not exist in the Chart of Accounts: ${missingAccounts.join(', ')}`);
    }
  }
  ...
  getCategoryAccountId(category: keyof AccountMappingConfig): string | null {
    ...
    if (!accountId || !this.accountsById.has(accountId)) {
      console.warn(`AccountMappingService: No valid account mapped for category "${String(category)}"`);
      return null;
    }
    return accountId;
  }
