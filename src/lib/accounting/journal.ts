...
import { areTransactionLinesBalanced, type TransactionLineInput } from '../validation/schemas/transaction';
...
export class Journal {
  ...
  addTransaction(transaction: Transaction): boolean {
    ...
    const linesForBalanceCheck: TransactionLineInput[] = transaction.lines.map(line => {
      ...
      if (typeof line.entity_account_id !== 'string') { // Ensure this property exists on TransactionLine
        console.error(`Transaction line for tx ${transaction.id} is missing 'entity_account_id'. Current line object:`, line);
        throw new Error(`Internal error: Transaction line missing 'entity_account_id'.`);
      }
      return {
        amount: line.amount,
  ...
  private calculateTotalByLines(
    ...
    this.transactions.forEach(transaction => {
      ...
      transaction.lines.forEach(line => { // line is inferred as TransactionLine
  ...
  getTotalDebits(onlyPosted: boolean = false): string {
    return this.calculateTotalByLines(line => line.is_debit, onlyPosted).toString();
  }
  ...
  isJournalBalanced(onlyPosted: boolean = false): boolean {
    const debits = this.calculateTotalByLines(line => line.is_debit, onlyPosted);
    const credits = this.calculateTotalByLines(line => !line.is_debit, onlyPosted);
