...
export async function calculateDepreciation(
  ...
  try {
    ...
    console.warn(
      "Mojo depreciation calculation failed, falling back to JS:",
      error
    );
  }
...
export async function generateDepreciationSchedule(
  ...
  try {
    ...
    console.warn(
      "Mojo depreciation schedule generation failed, falling back to JS:",
      error
    );
  }
...
function generateDepreciationScheduleJS(
  ...
  for (let year = 1; year <= yearsToCalculate; year++) {
    ...
    if (bookValue.lessThan(residualValue)) {
      ...
      const adjustedDepreciation = assetCost
        .minus(residualValue)
        .minus(accumulatedDepreciation.minus(depreciation));
...
export async function calculateTaxBasis(
  ...
  try {
    ...
    console.warn(
      "Mojo tax basis calculation failed, falling back to JS:",
      error
    );
  }
...
export async function calculateDepreciationRecapture(
  ...
  try {
    ...
    console.warn(
      "Mojo depreciation recapture calculation failed, falling back to JS:",
      error
    );
  }

  // Fallback to JavaScript implementation
  return calculateDepreciationRecaptureJS(
    originalCost,
