...
export async function generateAmortizationSchedule(
  ...
  try {
    ...
    console.warn(
      "Mojo amortization calculation failed, falling back to JS:",
      error
    );
  }

  // Fallback to JavaScript implementation
  return generateAmortizationScheduleJS(
    principal,
    annualRate,
    terms,
    paymentFrequency
...
function generateAmortizationScheduleJS(
  ...
  const p = new Decimal(principal);
  const r = new Decimal(annualRate).div(paymentFrequency);
  const payment = new Decimal(
    calculateLoanPaymentJS(principal, annualRate, terms, paymentFrequency)
  );
...
export async function compareLoanScenarios(
  ...
  try {
    ...
    console.warn(
      "Mojo loan scenario comparison failed, falling back to JS:",
      error
    );
  }
