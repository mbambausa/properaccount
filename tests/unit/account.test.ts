...
import { defaultChartOfAccounts } from "../../src/lib/accounting/core/chart-of-accounts.ts";
import type { ChartOfAccountsItem } from "../../src/lib/accounting/core/chart-of-accounts.ts";
...
const createMockDbChartOfAccount = (
  overrides: Partial<DbChartOfAccount> & {
    id: string;
    user_id: string;
    code: string;
    name: string;
  ...
  const numericIsRecoverable =
    typeof is_recoverable === "boolean"
      ? is_recoverable
        ? 1
        : 0
      : (is_recoverable ?? 0);
...
describe("AccountService", () => {
  ...
  describe("getAllAccounts", () => {
    it("should return all accounts for a user", async () => {
      ...
      const mockAccounts = mockAccountsData.map((e) =>
        createMockDbChartOfAccount(e)
      );
      mockDbQuery.mockResolvedValue(mockAccounts);

      const result = await accountService.getAllAccounts(testUserId);
      expect(mockDbQuery).toHaveBeenCalledWith(
        expect.stringMatching(/FROM chart_of_accounts WHERE user_id = \?/),
        [testUserId]
      );
  ...
  describe("getAccountById", () => {
    it("should return an account by its ID for the user", async () => {
      ...
      expect(mockDbQueryOne).toHaveBeenCalledWith(
        expect.stringMatching(/WHERE id = \? AND user_id = \?/),
        [mockAccountId, testUserId]
      );
  ...
  describe("updateAccount", () => {
    ...
    const existingAccountData = {
      ...
      code: "3010",
      name: "Old Equity",
      type: "equity" as AccountSystemType,
      is_active: 1,
    };
    const existingAccount = createMockDbChartOfAccount(existingAccountData);
    const updateData: Partial<ChartOfAccountInput> = {
    ...
    it("should update and return the account", async () => {
      ...
      expect(mockDbExecute).toHaveBeenCalledWith(
        expect.stringContaining(
          "UPDATE chart_of_accounts SET name = ?, is_active = ?, updated_at = ? WHERE id = ? AND user_id = ?"
        ),
  ...
  describe("deleteAccount", () => {
    ...
    it("should throw VALIDATION_ERROR if account has child accounts", async () => {
      ...
      mockDbQueryOne.mockResolvedValueOnce(
        createMockDbChartOfAccount(childAccountData)
      );
      ...
      await expect(
        ...
        new AppError(
          ...
          "Cannot delete account that is a parent to other accounts. Reassign child accounts first."
        )
      );
    });
  });
  ...
  describe("initializeDefaultAccounts", () => {
    it("should insert default accounts if none exist for the user", async () => {
      ...
      const mockD1Results: D1Result[] = defaultChartOfAccounts.map(
      ...
      expect(mockD1Prepare).toHaveBeenCalledTimes(
  ...
  describe("getAccountHierarchy", () => {
    it("should build and return a hierarchical structure of accounts", async () => {
      ...
      const childCashData = {
        ...
        name: "Cash",
        type: "asset" as AccountSystemType,
        parent_id: "asset-root-id",
      };
      const rootExpenseData = {
        id: "expense-root-id",
        user_id: testUserId,
        code: "5000",
        name: "Expenses",
      ...
      const assetsNode = hierarchy.find(
        (n) => n.id === "asset-root-id"
      ) as DbChartOfAccount & { children?: DbChartOfAccount[] };
